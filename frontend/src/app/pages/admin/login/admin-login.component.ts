...
@Component({
      ...
      <div class="login-card">
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()">
          <div class="form-group">
            <label>Email</label>
            <input type="email" formControlName="email" placeholder="admin@motors.com">
          </div>
          <div class="form-group">
  ...
  styles: [`
    ...
    .login-card {
      background: white; border-radius: var(--border-radius); padding: 3rem;
      width: 100%; max-width: 420px; box-shadow: var(--shadow-lg);
    }
    .login-header {
      text-align: center; margin-bottom: 2rem;
      h1 { color: var(--primary); font-size: 1.5rem; margin: 1rem 0 0.5rem; }
      p { color: var(--text-muted); font-size: 0.9rem; }
    }
    .form-group {
      margin-bottom: 1.25rem;
      label { display: block; font-size: 0.85rem; font-weight: 600; color: var(--primary); margin-bottom: 0.5rem; }
      input {
